...
const Item = ({ type, id, poster_path, title, vote_average, overview, release_date, first_air_date, original_name }) => (
  <div className="item">
    <Link to={`/${type}/${id}`}>
      {poster_path && <img src={baseURL + poster_path} alt={title || original_name} className="item-image" />}
    ...
    <div className="item-content">
      <div className="item-header">
        ...
        <div className="item-title">
          <h3 className="item-title-text">
            <Link to={`/${type}/${id}`} className="btn">{title || original_name}</Link>
          </h3>
      ...
      <p className="item-view-more">
        <Link to={`/${type}/${id}`} className="btn">more info</Link>
      </p>
    </div>
  </div>
);
