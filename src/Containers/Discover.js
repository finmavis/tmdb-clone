...
class Discover extends Component {
  ...
  loadData = async (toPage = 1, year = new Date().getFullYear(), sort = 'popularity.desc') => {
    this.toggleLoading();
  ...
  handleSelect = (newValue, name) => {
    ...
    this.props.history.push({
      pathname: url,
      search: queryString.stringify({...query, page: 1, [name]: newValue.value})
    });
  };
  
  render() {
    const { loading, page, total_pages, total_results, results, year, sort } = this.state;
    const { url } = this.props.match;
    const { type } = this.props.match.params;
    ...
    return (
      <section className="list-item">
        <Container>
          <Heading type="h2">
            discover new <Link to="/discover/movie" className={`btn btn-blue btn-discover ${type === 'movie' ? 'active' : ''}`}>movie</Link> or <Link to="/discover/tv" className={`btn btn-green btn-discover ${type === 'tv' ? 'active' : ''}`}>tv show</Link>
          </Heading>
          <SelectList 
            year={year}
            sort={sort}
